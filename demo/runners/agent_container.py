...
class AriesAgent(DemoAgent):
    ...
    async def generate_invitation(
        ...
        with log_timer("Generate invitation duration:"):
            ...
            log_status(
                "#7 Create a connection to The patient and print out the invite details"
            )
...
class AgentContainer:
    ...
    async def request_proof(self, proof_request, explicit_revoc_required: bool = False):
        log_status("#20 Request proof of degree from patient")
...
def arg_parser(ident: str = None, port: int = 8020):
    ...
    "ident", if specified, refers to one of the standard demo personas - patient, centerInv, acme or performance.
    """
    ...
    if (not ident) or (ident != "patient"):
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "Reuse connections by using CenterInv public key in the invite. "
...
async def test_main(
    ...
    centerInv_container = None
    patient_container = None
    try:
        # initialize the containers
        centerInv_container = AgentContainer(
            genesis_txns=genesis,
            ident="CenterInv.agent",
        ...
        patient_container = AgentContainer(
            genesis_txns=genesis,
            ident="Patient.agent",
            start_port=start_port,
            no_auto=no_auto,
        ...
        # start the agents - centerInv gets a public DID and schema/cred def
        await centerInv_container.initialize(
        ...
        await patient_container.initialize(
        ...
        # centerInv create invitation
        invite = await centerInv_container.generate_invitation()

        # patient accept invitation
        invite_details = invite["invitation"]
        connection = await patient_container.input_invitation(invite_details)

        # wait for centerInv connection to activate
        await centerInv_container.detect_connection()
        await patient_container.detect_connection()

        # TODO centerInv issue credential to patient
        # TODO patient check for received credential
        ...
        try:
            ...
            if patient_container:
                log_msg("Shutting down patient agent ...")
                await patient_container.terminate()
            if centerInv_container:
                log_msg("Shutting down centerInv agent ...")
                await centerInv_container.terminate()
        except Exception as e:
